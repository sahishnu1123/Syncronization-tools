...
void * t_func(void * t_pass){
    ...
    double l1 = data->l1;
    double l2 = data->l2;
    std::atomic<bool> *waiting = data->waiting;
    ...
    for(int p = 0; p < k; p++){
        ...
        ofstream ofile;
        ofile.open("output_casb.txt", ios::app);
        ...
        while(waiting[i] && key == 1){
            ...
            key.store(lock);
            lock.compare_exchange_strong(expected, 1);
            expected = 0;
        };
...
int main(int argc, char *argv[]){
    ...
    std::atomic <bool> *waiting = (std::atomic<bool> *)malloc(sizeof(std::atomic<bool>) * N);
    ...
    ofstream ofile;
    ofile.open("output_casb.txt");
    ofile<<"Bounded CAS ME Output\n\n";
    ofile.close();
